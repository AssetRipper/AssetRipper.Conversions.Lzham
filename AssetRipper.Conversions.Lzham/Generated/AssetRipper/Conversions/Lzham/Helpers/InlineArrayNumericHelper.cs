...
internal static partial class InlineArrayNumericHelper
{
	...
	public static TBuffer FMulAdd<TBuffer, TElement>(TBuffer x, TBuffer y, TBuffer z) where TBuffer : struct, IInlineArray<TElement> where TElement : IFloatingPointIeee754<TElement>
	{
		TBuffer buffer = default(TBuffer);
		TensorPrimitives.FusedMultiplyAdd(x.AsReadOnlySpan<TBuffer, TElement>(), y.AsReadOnlySpan<TBuffer, TElement>(), z.AsReadOnlySpan<TBuffer, TElement>(), buffer.AsSpan<TBuffer, TElement>());
		return buffer;
	}
}
